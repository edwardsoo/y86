...
public class CPU extends AbstractY86CPU.Sequential {
	...
	try {
		...
		case I_POPL:
		    ...
		    break;
		case I_CALL:
		    if (f.iFn.getValueProduced() == X_INDIRECT_CALL) {
			f.rA.set(mem.read(F.pc.get() + 1, 1)[0].value() >>> 4);
		    } else {
			f.rA.set(R_NONE);
		    }
	...
	if (D.stat.get() == S_AOK) {
		...
		case I_POPL:
		    ...
		    break;
		case I_CALL:
		    if (d.iFn.getValueProduced() == X_INDIRECT_CALL) {
			d.srcA.set(D.rA.get());
		    } else {
			d.srcA.set(R_NONE);
		    }
